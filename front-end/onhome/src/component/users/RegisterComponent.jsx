...
class RegisterComponent extends Component{
    ...
    insert=(e)=>{
        ...
        if(!user.id||!user.pw||!user.checkpw||!user.name||!user.email||!user.cod){
        ...
        else if(!this.state.canId||!this.state.canEmail){
            alert('아이디및 이메일 중복확인해주세요');
        }
            ...
            axios.get("http://localhost:8080/cod"+'/'+user.cod).then(res=>{
                ...
                if(cod){
                    ...
                    ApiService.insertUser(user).then(res=>{
                        alert('회원가입에 성공했습니다. 인증 및 승인전까지는 로그인이 불가능합니다');
           ...
           if(!this.state.canCo){
            ...
            .then(res=>{
                ...
                .then(res=>{
                    alert('회원가입에 성공했습니다. 인증 전까지는 로그인이 불가능합니다.');
                        this.props.history.push('/main');
                })
                .catch(err=>{
                    console.log('user post fail');
        ...
        .then(res=>{
            ...
            if(user){
                ...
                this.setState({
                  ...
                  axios.get("http://localhost:8080/users/email/"+this.state.user.email)
                  .then(res=>{
                      var user=res.data;
                      if(user){
                      ...
                      console.log('email duplicated check fail');
                  })  
                })
    ...
    <form style={{ paddingTop:"40px",paddingLeft:"250px"}}>
        ...
        <div>
            ...
            <label style={{display:"inline-block",position:"relative",width:"140px", height:"40px"}}>ID:</label>
             ...
             &nbsp;&nbsp;
             <button style={{display:"inline-block",position:"relative",width:"170px", height:"30px"}} type="button" class="btn btn-primary btn-sm " onClick={this.checkId}>이메일/아이디 중복확인</button>        
        </div>
